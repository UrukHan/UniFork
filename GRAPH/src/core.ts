...
function isCompleteMint(mintId: string): boolean {
    ...
    return mint !== null // sufficient checks
}
...
export function handleSync(event: Sync): void {
    ...
    bundle.ethPrice = ZERO_BD //getEthPriceInUSD()
    bundle.save()

    token0.derivedETH! = ZERO_BD //findEthPerToken(token0 as Token)
    token1.derivedETH! = ZERO_BD //findEthPerToken(token1 as Token)
    token0.save()
    token1.save()
    ...
    if (bundle.ethPrice.notEqual(ZERO_BD)) {
        trackedLiquidityETH = ZERO_BD //getTrackedLiquidityUSD(pair.reserve0, token0 as Token, pair.reserve1, token1 as Token).div(bundle.ethPrice)
    } else {
